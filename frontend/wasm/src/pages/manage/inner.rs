...
pub fn add_player(name: String, players: &[Player]) -> SessionDiff {
    let mut vec = players.to_vec();
    if let None = players.iter().find(|p| p.name == name) {
        vec.push(Player { name, score: 0 });
    }
    SessionDiff { stage: None, players: Some(vec) }
}

pub fn add_guess(name: String, players: &[Player]) -> SessionDiff {
    let mut vec = players.to_vec();
    if let Some(mut player) = vec.iter_mut().find(|p| p.name == name) {
        player.score += 1;
    }
    SessionDiff { stage: None, players: Some(vec) }
}

#[function_component(InnerManage)]
pub fn inner_manage(props: &Props) -> Html {
    let onadd = props.onchange.reform(|name| add_player(name, &props.session.players));
    let onguess = props.onchange.reform(|name| add_guess(name, &props.session.players));
    ...
    let body = match props.session.stage {
        ...
        Stage::Finished => {
            html! { <Rating quiz={self.props.quiz.clone()} />}
        },
    };
