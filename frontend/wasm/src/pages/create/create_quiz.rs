...
//         match msg {
//             Msg::Name(name) => self.new.name = name,
//             Msg::Creator(creator) => self.new.creator = creator,
//             Msg::Description(description) => self.new.description = description,
//             Msg::Upload(mut files) if files.len() == 1 => {
//                 self.new.image_url = Some(files[0].name());
//
...
//             Msg::Continue => {}
//             Msg::Read(Ok(bytes)) => {
//                 // self.new.image_bytes = Some(bytes);
//                 // TODO
//             }
//             Msg::Read(Err(err)) => {
//                 // give error
...
//
//     fn view(&self) -> Html {
//         let DraftQuiz { name, creator, description, image_name, .. } = &self.new;
//         let image_url = self.new.image_bytes.as_ref().map(bytes_to_url);
//
//         html! {
//             <pbs::Section>
//                 <pbs::Container>
